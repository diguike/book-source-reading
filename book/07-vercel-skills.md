...
一次 `skills add` 干的事分五个阶段，图 7-1 是整条主链路，后面每个模块都挂在这张图的某一段上。
...
这一节是全章最需要拆开看的一块，因为它有一个漂亮的机制和一个容易被误读的边界。
...
同一件事在两个作用域下需求完全不同，所以用了两个格式不同的锁文件：
...
内容哈希解决"变没变"，钉版本是另一件事。CLI 支持在来源后面用 `#` 加 ref（`src/source-parser.ts:203-233`）：
